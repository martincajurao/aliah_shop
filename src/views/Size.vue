  ...
  >
    ...
    <template v-slot:top>
        <v-dialog v-model="dialogDelete" max-width="500px">
          <v-card>
            <v-card-title class="headline">Do you want to delete this Size?</v-card-title>
            <v-card-actions>
              <v-spacer></v-spacer>
...
<script>
import {apiGetAllSizes,apiUpdateSizeData,apiCreateSize,apiDeleteSize,apiGetSize} from "@/api/size.api";
  ...
  export default {
    ...
    computed: {
      formTitle () {
        return this.editedIndex === -1 ? 'New Size' : 'Update Size'
      },
    },
    watch: {
      dialog (val) {
    ...
    methods: {
      ...
      deleteItemConfirm () {
        apiDeleteSize(this.editedItem.id).then(()=>{
      ...
      save () {
        ...
        if (this.editedIndex > -1) {
          apiUpdateSizeData(this.editedItem,this.editedItem.id).then(() => {
            ...
            this.$validator.validateAll().then(result => {
              if (result){
                apiCreateSize(this.editedItem).then(() => {
                  this.snackbar=true
                  this.tblLoader=true
                  this.close()
                  this.initialize()
